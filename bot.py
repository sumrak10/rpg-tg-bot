...
WEBHOOK_HOST = '62.84.123.173'
WEBHOOK_PORT = 443  # 443, 80, 88 или 8443 (порт должен быть открыт!)
WEBHOOK_LISTEN = '10.129.0.5'  # На некоторых серверах придется указывать такой же IP, что и выше
...
def get_prop_data_from_bd(id):
    conn = sqlite3.connect(dbAddress)
    cursor = conn.cursor()
    sql = "SELECT * FROM persons_prop WHERE id=?"
    cursor.execute(sql, [(id)])
    fetch_data = cursor.fetchall()
    fetch_data_reg = fetch_data[0]
    conn.close()
...
def insert_data_to_bd(id,data1,data2,data3,data4,data5):
    ...
    data = [(id, 0, data5, data4, '', '')]
    sql = """INSERT INTO persons_prop
                VALUES (?,?,?,?,?,?)"""
    cursor.executemany(sql, data)
    conn.commit()
    conn.close()
...
def update_mon_bd(usid, val):
	...
	conn.commit()
	conn.close()
def update_care_bd(usid, val):
	conn = sqlite3.connect(dbAddress)
	cursor = conn.cursor()
	data = [(val, usid)]
	sql = """UPDATE persons_prop 
			SET care = ?
			WHERE id = ?"""
	cursor.executemany(sql, data)
	conn.commit()
	conn.close()
def update_coming_bd(usid, val):
	conn = sqlite3.connect(dbAddress)
	cursor = conn.cursor()
	data = [(val, usid)]
	sql = """UPDATE persons_prop 
